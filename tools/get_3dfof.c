...
int get_files_to_read_filesofgroup (char * prefix, int nstructs, int * strct_id, int * files_to_read);
...
/*
    float hbox =  50000 * 0.9823335 / 0.704;
    float Lbox = 100000 * 0.9823335 / 0.704;
    ...
    for (i = 0; i < acum; i++)
    {
      ...
      while (p[i].Pos[0] >  hbox) p[i].Pos[0] -= Lbox;
      while (p[i].Pos[1] >  hbox) p[i].Pos[1] -= Lbox;
      while (p[i].Pos[2] >  hbox) p[i].Pos[2] -= Lbox;
    ...
    for(k = 0; k < 6; k++)
      if((header.npart[k] > 0) && (header.mass[k] == 0))
        for(i = 0; i < particles; i++)
          if (pg[i].Type == k)
            fwrite(&pg[i].Mass, sizeof(float), 1, snap_file);
    fwrite(&dummy, sizeof(dummy), 1, snap_file);
  }
...
int get_files_to_read_filesofgroup (char * prefix, int nstructs, int * strct_id, int * files_to_read)
{
  ...
  for (i = 1; i <= nstructs; i++)
  {
    ...
    if (strct_id[tmpid])
    {
      ...
      for (j = 0; j < nfiles; j++)
        files_to_read[files_of_strct[j]]++;
      free (files_of_strct);
    }
  }
